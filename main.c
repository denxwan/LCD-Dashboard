...
// --- Pre-transfer callback for DC pin ---
...
static void lvgl_flush_cb(lv_disp_drv_t *disp, const lv_area_t *area, lv_color_t *color_map) {
    ...
    for (row = 0; row < h; row += max_rows) {
        ...
        uint8_t *buf_ptr = (uint8_t *)&color_map[row * w];
        ...
        uint16_t y2 = y1 + rows_to_send - 1;
        uint8_t row_addr[] = { (y1>>8)&0xFF, y1&0xFF, (y2>>8)&0xFF, y2&0xFF };
        st7701s_cmd(0x2B);
        st7701s_data(row_addr, 4);

        // Write RAM
        st7701s_cmd(0x2C);
        st7701s_data(buf_ptr, w * rows_to_send * 2);
    }
